-- Deploy eed:database_functions/verify_grants to pg
-- requires: schema_eed

begin;

-- Verifies permission grants for all columns in a table
create or replace function eed_private.verify_grant(
  operation text,
  table_name_input text,
  role_name text,
  table_schema_name text default 'eed'
)
returns boolean
as
$function$
...
comment on function eed_private.verify_grant(text, text, text, text) is
  'A generic function for testing the existence of grants on a table';

-- Verifies permission grants on speeedic columns in a table
create or replace function eed_private.verify_grant(
  operation text,
  table_name_input text,
  role_name text,
...
comment on function eed_private.verify_grant(text, text, text, text[], text) is
